<template>
  <div class="article">
    ...
    <div class="content">
      <div class="cont">
        <h3>{{json.title}}</h3>
        <div class="time"><p>{{json.time|date}} <span><img :src="json.author_face" alt=""/></span></p></div>
        <div class="text-box" v-html="json.content"></div>
      </div>
    </div>
    <div class="foot-btn">
      <ul>
...
<script>
  export default{
    ...
    methods:{
      go(n){
        this.$router.go(n); //返回路由
      }/*,
      get(){
        alert(this.$route.params.aid);//取路由参数
      }*/
    },
    mounted(){
      //this.$route.params.aid
      this.$http.get('/src/data/article.data')
        .then((res)=>{
          this.json=res.data[this.$route.params.aid-1];
        },(error)=>{
          alert('article.vue'+error);
        });
    }
  }
</script>
