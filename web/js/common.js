	...
	_getTimeDaysLater: function(days) {
		...
		if(/^[1-9]+[0-9]*]*$/.test(days)) {
			var toInt = days * 1 | 0 || 0, //将天数做整数处理
				...
				nowTimestemp = TimeUtility._timestemp();
			return TimeUtility._getDateHasHoursMinutesSecondes(second + nowTimestemp);
		} else {
			alert("请输入整数！");
		}
	}
	...
	_checkID:function(val) {
		if(FormUtility._checkCode(val)) {
			...
			if(FormUtility._checkDate(date)) {
				if(FormUtility._checkProv(val.substring(0, 2))) {
					return true;
				}
			}
		}
	...
	_isEmptyArray: function(params) {
		...
		if(params.length > 0) {
			for(var i = 0; i < params.length; i++) {
				if(EmptyUtility._isEmpty(params[i])) {
					flag = true;
					break;
				}
			}
